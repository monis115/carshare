...
if(move_uploaded_file($tmp_name,$permanentDestination))
{
    ...
    if(!$result)
    {
        echo " <div class='alert alert-danger'> unable to upate the profile picture Please try again later !</div>";
    }
}else
{
    echo " <div class='alert alert-danger'> unable to upate the profile picture Please try again later !</div>";
...
if($fileError>0)
{
    echo " <div class='alert alert-danger'> unable to upate the profile picture Please try again later ! Error Code : $fileError </div>";
}
// session_start();
// include('connection.php');
...
//     $permanentdestination = 'profile/' . md5(time()) . ".$ext";
